...
parse_line    _          []  = []
parse_line (x,y,z) ('.':txt) = ((x,y,z), False) : parse_line (x+1,y,z) txt
parse_line (x,y,z) ('#':txt) = ((x,y,z), True) : parse_line (x+1,y,z) txt
parse_line    _    ( c :txt) = error $ "Unexpected symbol " ++ [c]

parse_lines :: Coordinate -> [String] -> FinWorld
parse_lines    _       []  = []
parse_lines (x,y,z) (l:ls) = parse_line (x,y,z) l ++ parse_lines (x,y+1,z) ls

get_finWorld = readFile inputF >>= return . parse_lines (0,0,0) . lines
...
get_starting_coordinates = do
  ...
  return [(x,y,0) | x <- [0..l_l], y <- [0..l_c]]

coordinates_around :: [Coordinate] -> [Coordinate]
coordinates_around cs = [(x,y,z) | x <- [min_x - 1 .. max_x + 1]
                                 , y <- [min_y - 1 .. max_y + 1]
                                 , z <- [min_z - 1 .. max_z + 1]]
  where
    min_x = minimum [x | (x,_,_) <- cs]
    max_x = maximum [x | (x,_,_) <- cs]
    min_y = minimum [y | (_,y,_) <- cs]
    max_y = maximum [y | (_,y,_) <- cs]
    min_z = minimum [z | (_,_,z) <- cs]
    max_z = maximum [z | (_,_,z) <- cs]


next_state :: World -> Coordinate -> Bool
next_state f c@(x,y,z)
  | f c = (sum $ map fromEnum $ [f (x',y',z') | x' <- [x-1..x+1]
                                              , y' <- [y-1..y+1]
                                              , z' <- [z-1..z+1]]) `elem` [3,4]
  | otherwise = (sum $ map fromEnum $ [f (x',y',z') | x' <- [x-1..x+1]
                                                    , y' <- [y-1..y+1]
                                                    , z' <- [z-1..z+1]]) == 3
